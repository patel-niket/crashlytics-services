...
class Service::GitHubEnterprise < Service::Base
    title 'GitHub Enterprise'
    
    string :api_endpoint, :placeholder => 'hostname', :label => 'Your Enterprise GitHub Hostname API URL ForExample, github.company.com/api/v3/'
    
    string :repo, :placeholder => 'org/repository', :label => 'Your Enterprise GitHub repository.'
    ...
    page 'API Endpoint', [:api_endpoint]
    page 'Repository', [:repo]
    page 'Access token', [:access_token]
    
    def receive_verification(config, _)
        repo = github_repo(config[:access_token], config[:api_endpoint], config[:repo])
    ...
    def create_github_issue(access_token, api_endpoint, repo, title, body)
